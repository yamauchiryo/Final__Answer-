...
url = ["https://r.gnavi.co.jp/rhuymzbd0000/?sc_lid=home_check_shop",
       ...
       "https://r.gnavi.co.jp/fg2dagcv0000/",
       "https://r.gnavi.co.jp/a781x9p40000/",
...
for link in url: #urlリストに格納されているurlを1つずつ取り出して情報を抽出する
    time.sleep(3) #3秒間のアイドリング
    res = requests.get(link)
    soup = BeautifulSoup(res.content, "html.parser")

    shop_name_element = (soup.find("p", attrs={"class": "fn org summary"}))
    tele_number_element = (soup.find("span", attrs={"class": "number"}))
    mail_address = None
    address_element = (soup.find("span", attrs={"class": "region"}).text)
    building_element = (soup.find("span", attrs={"class": "locality"}))
    official_page_link_element = (soup.find('a', {'class': 'sv-of double', 'title': 'オフィシャルページ'}))['href']

    shop_list = []
    list_append(shop_list,shop_name_element)
    list_append(shop_list,tele_number_element)
    shop_list.append(mail_address)
    result = parse_address(address_element)
    address_append(result)
    list_append(shop_list,building_element)
...
df.to_csv("1-1.csv", index_label='1', header=True)
